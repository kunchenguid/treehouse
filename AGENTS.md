...
- Dirty checks include untracked files even when repository config hides them from normal `git status` output (`IsDirty` runs `git status --porcelain --untracked-files=all`). Files ignored by `.gitignore` are NOT counted as dirty, so a worktree whose only leftover content is ignored is treated as clean and can be reused by `Acquire`/`prune`/`destroy` defaults; but `ResetWorktree` runs `git clean -fd` (no `-x`), which respects `.gitignore`, so those ignored files survive the reset rather than being deleted
