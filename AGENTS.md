...
- Exit statuses are mapped in `cmd/exit.go`: any error out of `Execute` is `ExitFailure` (1) unless a command tagged it with `withExitCode`. `return` tags its aborts `ExitNotReturned` (3) so a caller can tell "dirty, still held" from a genuine failure — an abort left the lease in place while reporting success, which silently starved pools under automation. A command that leaves the world unchanged must never exit 0. 2 stays free for usage errors
