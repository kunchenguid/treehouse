...
- `cmd/` - CLI commands (cobra): `get` (incl. `get --lease`), `current`, `enter`, `return`, `status`, `prune`, `destroy`
...
- `current` (see `currentRunE` + `pool.Current`) answers "is this directory inside a managed worktree?" authoritatively: it derives the owning repo with `git.FindMainRepoRootFrom` (so pool resolution is correct from inside a linked worktree, the same class of bug PR #51 fixes for other commands), resolves the pool, and matches the cwd against pool state via `pool.Current` — it does NOT trust `$TREEHOUSE_DIR` (which only the `get` subshell exports). `pool.Current` guards on `IsPoolDir` so it never creates a pool dir as a side effect. Exit code is the contract: 0 inside a worktree, non-zero outside; `--path` prints only the path (silent when outside) and `--json` emits `{"in_worktree":...}` (JSON `--json` output shape inspired by the follow-up in PR #47). Non-zero "not here" exits use the `errSilent` sentinel so `Execute` skips extra stderr decoration
