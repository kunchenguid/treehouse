...
- Only `return` clears leases; a recovered slot is released only by a named `return <path>` (which warns seeded files may remain), or removed with `destroy <path> --include-leased --yes`; `return --all` skips it.
