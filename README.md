...
| `get`     | `--cow-cache` | Opt in to selected, allowlisted cache reflinks (no byte-copy fallback) |
...
`get --cow-cache` (also with `--lease`) instead clones only regular ignored files selected by that same manifest **and** under these cache roots: `.cache/go-build/`, `.cache/uv/`, `node_modules/.cache/`, `target/{debug,release}/incremental/`, `.gradle/caches/`, `.next/cache/`. It excludes `.env`/`.env.*`, credentials, secrets, Firstmate private `data`, `state`, `config`, and nested `.git`/`.jj` worktrees, even if a manifest selects them. Inspect the manifest before opting in: this is for reproducible non-secret caches only. On APFS it uses copy-on-write per-file clones; on other platforms and filesystems without clone support, selected caches stay cold **without a byte-copy fallback**. The ordinary seeding mode remains unchanged when this flag is absent; use it for small ignored config files, not bulk caches. The initial allowlist is intentionally narrow; add cache types only after reviewing their contents and tests. Clone errors other than unsupported/cross-device fail acquisition and follow the same cleanup/quarantine path as normal seeding. Cloned files remain inventoried and are removed on return or reuse; previously task-generated ignored cache files outside that inventory are left untouched and not overwritten on reuse. Unsupported clone attempts remove empty directories they created. The option does not change lease or process safety and is not enabled by config.
