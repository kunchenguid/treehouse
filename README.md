...
The native implementation hashes the original destination and source, clones into destination-local staging, verifies the staged bytes, restores and verifies destination permissions/owner/group/timestamps/xattrs, hashes again, and atomically replaces the destination. Inode and ctime change; creation time is not preserved. Symlinked paths, ACLs, hardlinks, special mode/flag bits, sparse/compressed representations, different owners, and unsupported metadata are skipped. Other operating systems, non-APFS filesystems and cross-volume pairs keep ordinary copies, with the reason on stderr. No Python helper or daemon is required.
