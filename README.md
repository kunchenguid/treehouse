...
- **Leased to someone else since.** `--all` lists the pool once and then works through it, so an earlier confirmation can hold the run open while a later slot changes hands. Each release is pinned to the lease the listing saw: a slot that was leased is refused if that lease identity changed, and a slot that was not leased is refused if it has been leased since. That is the whole guarantee - a slot handed to another plain `treehouse get` carries no lease to compare, so it is returned like any other in-use slot, which is what `--all` does to in-use slots by design.
