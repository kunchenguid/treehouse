...
// seedAuthDirFor is the shared authentication directory gitvcs keeps beside a
// jj worktree. Reproducing the path here (rather than exporting it) keeps the
// assertion on the observable litter, which is what the test is about.
func seedAuthDirFor(worktreePath string) string {
	return filepath.Join(filepath.Dir(worktreePath), ".treehouse-jj-seed-auth")
}

// registerSeededJJSlotOutsidePool builds the shape the built-in layout never
// produces: a jj slot worktree_path placed beside a repository, so its seed
// authentication lands in a directory the user owns rather than in a numbered
// slot directory the pool deletes wholesale. It returns the worktree path.
func registerSeededJJSlotOutsidePool(t *testing.T, poolDir, worktree, storePath string) string {
	t.Helper()
	marker := filepath.Join(worktree, ".jj", "repo")
	if err := os.MkdirAll(filepath.Dir(marker), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(marker, []byte(storePath), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(worktree, "selected.env"), []byte("seeded\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := gitvcs.PrepareJJSeededCleanup(worktree); err != nil {
		t.Fatal(err)
	}
	entry := WorktreeEntry{Name: "1", Path: worktree}
	...
	entries, err := os.ReadDir(seedAuthDirFor(worktree))
	if err != nil || len(entries) != 1 {
		t.Fatalf("seed authentication entries = %v, %v", entries, err)
	}
	state, err := ReadState(poolDir)
	if err != nil {
		t.Fatal(err)
	}
	if len(state.Worktrees) != 1 || state.Worktrees[0].SeedBackend != "jj" || state.Worktrees[0].SeedAuthIdentity == "" {
		t.Fatalf("expected a signed jj seed entry, got %#v", state.Worktrees)
	}
	return worktree
}

// TestDestroyWorktree_MarkerlessSlotOutsidePoolRemovesJJSeedAuthentication pins
// the cleanup for destroy's plain-directory route. A markerless slot never
// reaches vcs.RemoveWorktree, and outside the pool only the worktree itself is
// deleted, so the seed authentication has to be dropped explicitly. Left behind
// it keeps the shared directory permanently non-empty, which defeats the
// empty-directory cleanup for every other slot beside it too.
func TestDestroyWorktree_MarkerlessSlotOutsidePoolRemovesJJSeedAuthentication(t *testing.T) {
	base := t.TempDir()
	base, err := filepath.EvalSymlinks(base)
	if err != nil {
		t.Fatal(err)
	...
	checkouts := filepath.Join(base, "checkouts")
	store := filepath.Join(checkouts, "myrepo", ".jj", "repo")
	if err := os.MkdirAll(store, 0o755); err != nil {
		t.Fatal(err)
	}
	worktree := registerSeededJJSlotOutsidePool(t, poolDir, filepath.Join(checkouts, "myrepo-1"), store)
	if err := os.RemoveAll(filepath.Join(worktree, ".jj")); err != nil {
		t.Fatal(err)
	}

	result, err := DestroyWorktree(poolDir, worktree, DestroyOptions{IncludeUnlanded: true})
	if err != nil {
		t.Fatalf("DestroyWorktree failed: %v", err)
	}
	if len(result.Destroyed) != 1 {
		t.Fatalf("expected the markerless slot removed, got %#v (skipped: %#v)", result.Destroyed, result.Skipped)
	}
	if _, err := os.Stat(worktree); !os.IsNotExist(err) {
		t.Fatalf("expected worktree removed, stat err = %v", err)
	}
	if _, err := os.Stat(seedAuthDirFor(worktree)); !os.IsNotExist(err) {
		t.Fatalf("jj seed authentication survived removal: %v", err)
	}
	if _, err := os.Stat(filepath.Join(checkouts, "myrepo")); err != nil {
		t.Fatalf("the directory holding the worktree must survive intact: %v", err)
	}
}

// TestPrune_OrphanOutsidePoolRemovesJJSeedAuthentication pins the same cleanup
// for prune's other plain-directory route: a workspace whose backing store is
// gone is removed with --prune-orphans without going through the backend.
func TestPrune_OrphanOutsidePoolRemovesJJSeedAuthentication(t *testing.T) {
	base := t.TempDir()
	base, err := filepath.EvalSymlinks(base)
	if err != nil {
		t.Fatal(err)
	...
	deletedStore := filepath.Join(checkouts, "myrepo", ".jj", "repo")
	worktree := registerSeededJJSlotOutsidePool(t, poolDir, filepath.Join(checkouts, "myrepo-1"), deletedStore)

	result, err := PruneWithOptions(filepath.Join(checkouts, "myrepo"), poolDir, PruneOptions{PruneOrphans: true})
	if err != nil {
		t.Fatalf("PruneWithOptions failed: %v", err)
	}
	if len(result.Pruned) != 1 || result.Pruned[0].Path != worktree {
		t.Fatalf("expected the orphan pruned, got %#v (skipped: %#v)", result.Pruned, result.Skipped)
	}
	if _, err := os.Stat(worktree); !os.IsNotExist(err) {
		t.Fatalf("expected worktree removed, stat err = %v", err)
	}
	...
	if _, err := os.Stat(checkouts); err != nil {
		t.Fatalf("the directory holding the worktree must survive: %v", err)
	}
}
